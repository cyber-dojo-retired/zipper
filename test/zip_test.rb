...
class ZipTest < ZipperTestBase
  ...
  test '561',
  'info from unzipped files matches info from saver' do
    started_kata_ids = started_kata_args.map { |args| args[0] }
    (started_kata_ids + [ unstarted_kata_id ]).each do |id|
      encoded = zip(id)
      assert_unzip_matches_saver(id, encoded)
    end
  end

  private # = = = = = = = = = = = = = = = = = =

  def assert_unzip_matches_saver(id, encoded)
    Dir.mktmpdir('zipper') do |tmp_dir|
      ...
      saver_manifest = saver.kata_manifest(id)
      diagnostic = "#{id} manifests are NOT the same"
      assert_equal saver_manifest, zipper_manifest, diagnostic

      saver.avatars_started(id).each do |avatar_name|
        avatar_path = "#{kata_path}/#{avatar_name}"
        zipper_rags = disk[avatar_path].read_json('increments.json')
        saver_rags = saver.avatar_increments(id, avatar_name)
